...
from home_library.vectorizer import vectorize_epub, VectorizationResult, TextChunk
...
class EmbeddingsCreator:
    ...
    def create_embeddings_for_epub(
        ...
        logger.info(f"Creating embeddings for {len(texts)} chunks using batch size {self.batch_size}")
        ...
        embedding_chunks = []
        for i, (chunk, embedding) in enumerate(zip(vectorization_result.chunks, embeddings)):
    ...
    def get_embeddings_stats(self, result: EmbeddingsResult) -> dict[str, Any]:
        ...
        return {
            ...
            "efficiency": {
                "chunks_per_second": round(result.total_chunks / result.processing_time_seconds, 2),
                "words_per_second": round(result.total_words / result.processing_time_seconds, 2)
            }
        }
